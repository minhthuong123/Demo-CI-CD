...
class Control {
  ...
  async _wait_for_element_displayed(element_locator, time_out_milliseconds = 0) {
    if (time_out_milliseconds == 0) {
      time_out_milliseconds = await this.setDefaultTimeOut();
    }
    try {
      await browser.$(element_locator).waitForDisplayed({ timeout: time_out_milliseconds, reverse: false, timeoutMsg: '[ERR] Wait For Display:' + element_locator });
      return true;
    } catch (err) {
      return false;
    }
  ...
  async _wait_for_element_disappear(element_locator, time_out_milliseconds = 0) {
    ...
    try {
      await browser.$(element_locator).waitForDisplayed({ timeout: time_out_milliseconds, reverse: true, timeoutMsg: '[ERR] Wait For Disappear:' + element_locator });
      return true;
    } catch (err) {
      return false;
    }
  ...
  async _wait_for_element_clickable(element_locator, time_out_milliseconds = 0) {
    ...
    try {
      await browser.$(element_locator).waitForClickable({ timeout: time_out_milliseconds, reverse: false, timeoutMsg: '[ERR] Wait For Clickable:' + element_locator });
      return true;
    } catch (err) {
  ...
  async _wait_for_element_enabled(element_locator, time_out_milliseconds = 0) {
    if (!time_out_milliseconds) {
      time_out_milliseconds = await this.setDefaultTimeOut();
    }
    return await browser.$(element_locator).waitForEnabled({ timeout: time_out_milliseconds, reverse: false, timeoutMsg: '[ERR] Element ' + element_locator + ' is not displayed' });
  }
  ...
  async _wait_for_element_exist(element_locator, time_out_milliseconds = 0) {
    ...
    return await browser.$(element_locator).waitForExist({ timeout: time_out_milliseconds });
  }
  ...
  async _find_element(element_locator, time_out_milliseconds = 0) {
    ...
    return await browser.$(element_locator);
  }
  ...
  async _is_element_displayed(element_locator, time_out_milliseconds = 0) {
    ...
    // try {
    //     return browser.$(element_locator).isDisplayed()
    // } catch (e) {
  ...
  async _click_element(element_locator, time_out_milliseconds = 0) {
    ...
    try {
      ...
      return await browser.$(element_locator).click();
    } catch (err) {
  ...
  async _hover_element(element_locator, time_out_milliseconds = 0, ele_disabled = false) {
    ...
    try {
      await browser.$(element_locator).moveTo();
  ...
  async _scroll_to_view(element_locator, time_out_milliseconds = 0, scroll_into_view_options = 'false') {
    await this._wait_for_element_displayed(element_locator, time_out_milliseconds);
    await browser.$(element_locator).scrollIntoView(scroll_into_view_options);
  }
  ...
  async _set_value(element_locator, value, time_out_milliseconds = 0) {
    ...
    try {
      if (process.env.BROWSER == 'chrome' || process.env.BROWSER == 'chromium') {
        let ele = await browser.$(element_locator);
      ...
      // await browser.$(element_locator).click()
      await browser.$(element_locator).setValue(value);
    } catch (err) {
  ...
  async _add_value(element_locator, value) {
    await browser.$(element_locator).addValue(value);
  }
  ...
  async _clear_value(element_locator, time_out_milliseconds = 0) {
    ...
    try {
      if (process.env.BROWSER == 'chrome' || process.env.BROWSER == 'chromium') {
        ...
        return await browser.$(element_locator).clearValue();
      }
    } catch (err) {
  ...
  async _get_text(element_locator, time_out_milliseconds = 0) {
    try {
      await this._wait_for_element_displayed(element_locator, time_out_milliseconds);
      return await browser.$(element_locator).getText();
    } catch (err) {
  ...
  async _get_value(element_locator, time_out_milliseconds = 0) {
    try {
      await this._wait_for_element_clickable(element_locator, time_out_milliseconds);
      return await browser.$(element_locator).getValue();
    } catch (err) {
  ...
  async _get_attribute_value(element_locator, attribute_name, time_out_milliseconds = 0) {
    try {
      await this._wait_for_element_clickable(element_locator, time_out_milliseconds);
      return await browser.$(element_locator).getAttribute(attribute_name);
    } catch (err) {
  ...
  async _right_click_element(element_locator, time_out_milliseconds = 0) {
    try {
      await this._wait_for_element_clickable(element_locator, time_out_milliseconds);
      await browser.$(element_locator).click({ button: 'right' });
    } catch (err) {
  ...
  async _click_element_by_coordinates(element_locator, x_coordinates = 0, y_coordinates = 0, time_out_milliseconds = 0) {
    ...
    try {
      await this._wait_for_element_displayed(element_locator, time_out_milliseconds);
      await browser.$(element_locator).click({ x: x_coordinates, y: y_coordinates });
    } catch (err) {
  ...
  async _switch_to_window_id(window_id) {
    ...
    await browser.switchToWindow(windows[window_id]);
  }
  ...
  async _switch_to_iframe(element_locator) {
    // id = browser.$(element_locator)
    await browser.switchToFrame(await browser.$(element_locator));
  }

  async _drag_and_drop(element_locator, element_target_locator) {
    let source_ele = await browser.$(element_locator);
    let target_ele = await browser.$(element_target_locator);
  ...
  async _drag_and_drop_by_coordinates(element_locator, x_coordinates, y_coordinates) {
    ...
    await browser.$(element_locator).dragAndDrop({ x: x_coordinates, y: y_coordinates });
  }

  async _get_coords_for_element(elementId) {
    // elementID = browser.$(ele_locator).elementId
  ...
  async _element_existing(element_locator) {
    return await browser.$(element_locator).isExisting();
  }
